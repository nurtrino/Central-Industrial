...
Two external seams are reimplemented here so it runs standalone in this platform:
  * llm_core.py  — routes the engine's LLM calls to our Anthropic key (claude-fable-5,
                   high effort), so a comparison isolates METHODOLOGY rather than model choice.
